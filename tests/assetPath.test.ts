...
describe('versioned', () => {
  it('appends the content hash of a photo in the manifest', () => {
    expect(versioned('/photos/main.png')).toMatch(/^\/photos\/main\.png\?v=[0-9a-f]{8}$/)
  })

  it('gives different photos different versions', () => {
    expect(versioned('/photos/main.png')).not.toBe(versioned('/photos/meta-image.png'))
  })
...
describe('absoluteAsset', () => {
  it('builds a versioned absolute URL for share/OG tags', () => {
    expect(absoluteAsset('https://example.com/wedding/', '/photos/meta-image.png')).toMatch(
      /^https:\/\/example\.com\/wedding\/photos\/meta-image\.png\?v=[0-9a-f]{8}$/,
    )
  })

  it('does not double up the slash when the base has no trailing slash', () => {
    expect(absoluteAsset('https://example.com', '/photos/meta-image.png')).toMatch(
      /^https:\/\/example\.com\/photos\/meta-image\.png\?v=/,
    )
  })
})
